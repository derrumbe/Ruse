...
//  StyleTransfer.swift
//
// Transers style to current image
//
...
class StyleTransferer {
  ...
  static func newCPUStyleTransferer(
   ...
   /// Create a Style Transferer instance with a Float16 model that runs inference on a GPU.
  ...
  // MARK: - calculateBlendedBottleneck

  /// Calculates the blended bottleneck for this transfer
  /// - Parameters
  ///   - firstBottleneck: first data item to add
  ///   - secondBottleneck: second data item to add
  /// - Returns: blendedBottleneck
    //func calculateBlendedBottlenck {
   // }
  ...
  func runStyleTransfer(style styleImage: UIImage,
    ...
    tfLiteQueue.async {
      ...
      do {
        ...
        guard
          let inputRGBDataStyleScale = image.scaledData(
            with: Constants.styleImageSize,
            isQuantized: false
          )
          else {
        ...
        let bottleneckStyle = predictResultTensor.data
        
        // need to shrink the content image down to 256 before preocessing
        // CONTENT for blending / Copy the RGB data to the input `Tensor`.
        try self.predictInterpreter.copy(inputRGBDataStyleScale, toInputAt: 0)

        // CONTENT for blending / Run inference by invoking the `Interpreter`.
        try self.predictInterpreter.invoke()

        // CONTENT for blending / Get the output `Tensor` to process the inference results.
        let predictResultTensorInput = try self.predictInterpreter.output(at: 0)

        // Grab bottleneck data from output tensor.
        let bottleneckInput = predictResultTensorInput.data

        //how much of hte original do we want? (1.0 is all content)
        //let contentBlendRatio : Float = 0.5

       //let bottleneckBlended=bottleneck
        // convert new BLENDED bottleneck

        print(predictResultTensor.dataType)
        print(predictResultTensor.shape)
        print(predictResultTensor.data)
        
        
        let blendedBottleneck = calculateBlendedBottleneck(dataInput: bottleneckInput, dataStyle: bottleneckStyle, blendRatio: contentBlendRatio)
        
//        var bottleneckBlended = (content_blending_ratio * bottleneckInput)// + ((1-content_blending_ratio) * bottleneck)
        ...
        //adjusting this for blending purposes
        try self.transferInterpreter.copy(blendedBottleneck!, toInputAt: 1)
...
/// Calculates the blended data structure to account for a blending of target and style impacts
///  Creating a one-off dedicated function to handle the Data arithmetic
/// - Parameter dataInput: The bottleneck created from the input (original image). This data must be a buffer of
///   `Float32` values between 0 and 1 in RGB format.
/// - Parameter dataStyle: The bottleneck from the style image`. This data must be a buffer of
///   `Float32` values between 0 and 1 in RGB format.
/// - Parameter blendRatio: The Double that represents how to balance the input and style bottlenects
private func calculateBlendedBottleneck(dataInput: Data,
                                  dataStyle: Data,
                                  blendRatio: Float) -> Data? {

    let floatsInput = dataInput.toArray(type: Float32.self)
    let floatsStyle = dataStyle.toArray(type: Float32.self)
    
    var newBottleneck : Array<Float32> = Array(repeating: 0, count: floatsInput.count)
    ...
    let unsafePointer = UnsafeMutablePointer<Float32>.allocate(capacity: bufferCapacity)
    let unsafeBuffer = UnsafeMutableBufferPointer<Float32>(start: unsafePointer,
                                                         count: bufferCapacity)
    defer {
      unsafePointer.deallocate()
    }

    for i in 0..<floatsInput.count {
        // this is the formula we're using to calculate the blend ratio
//        var bottleneckBlended = (content_blending_ratio * bottleneckInput)// + ((1-content_blending_ratio) * bottleneck)
        newBottleneck[i] = blendRatio * floatsInput[i] + (1-blendRatio) * floatsStyle[i]
        unsafeBuffer[i] = newBottleneck[i]
    }
    ...
    //var newData : Data = floatsInput.
   // need to mod this later
    return outData
}
